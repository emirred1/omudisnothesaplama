...
export default function Home() {
  ...
  const calculateResults = () => {
    ...
    const yilIciAğırlıklı = (n(k1) * 0.22) + (realK2 * 0.18) + (realK3 * 0.30) + (realK4 * 0.16) + (n(dsbb) * 0.10) + (n(tdp) * 0.04);
    ...
    return {
      ...
      isImpossible,
      isPassing: genelNot >= 59.5 && n(final) >= 50
    };
  };
      ...
      <div className={`w-full max-w-md flex-grow flex flex-col justify-center py-10 transition-all duration-500 ${shareMode ? 'scale-110' : 'scale-100'}`}>
        ...
        {!shareMode && (
          <div className="space-y-4 text-left">
            <div>
              ...
              <input type="number" placeholder="0" value={scores.k1} onChange={(e) => handleInputChange('k1', e.target.value)} className={`w-full border-none rounded-2xl p-4 text-lg font-medium outline-none transition-all placeholder:text-zinc-600 ${darkMode ? 'bg-zinc-900 text-white focus:ring-2 focus:ring-zinc-700' : 'bg-white shadow-sm focus:ring-2 focus:ring-zinc-200'}`} />
            </div>
            ...
            {[2, 3, 4].map(num => (
              <div key={num} className={`grid grid-cols-2 gap-3 p-3 rounded-2xl border transition-colors ${darkMode ? 'bg-zinc-900/50 border-zinc-800' : 'bg-white border-zinc-100 shadow-sm'}`}>
                <div>
                  ...
                  <input type="number" placeholder="0" value={scores[`k${num}` as keyof typeof scores]} onChange={(e) => handleInputChange(`k${num}`, e.target.value)} className={`w-full border-none rounded-xl p-3 text-base font-semibold outline-none transition-all ${darkMode ? 'bg-zinc-800 text-white' : 'bg-zinc-50'}`} />
                </div>
                <div>
                  <label className="block text-[9px] font-black text-zinc-400 uppercase mb-1 ml-1 tracking-tighter">K{num} PDÖ</label>
                  <input type="number" placeholder="0" value={scores[`k${num}_pdo` as keyof typeof scores]} onChange={(e) => handleInputChange(`k${num}_pdo`, e.target.value)} className={`w-full border-none rounded-xl p-3 text-base font-semibold outline-none transition-all ${darkMode ? 'bg-zinc-800/40 text-zinc-500' : 'bg-zinc-100/50 text-zinc-400'}`} />
                </div>
              </div>
            ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" placeholder="0" value={scores.dsbb} onChange={(e) => handleInputChange('dsbb', e.target.value)} className={`w-full border-none rounded-xl p-3 text-base font-medium outline-none transition-all ${darkMode ? 'bg-zinc-900 text-white' : 'bg-white shadow-sm'}`} />
              </div>
              <div>
                <label className="block text-[9px] font-black text-zinc-500 uppercase mb-1 ml-1 tracking-widest">TDP (%4)</label>
                <input type="number" placeholder="0" value={scores.tdp} onChange={(e) => handleInputChange('tdp', e.target.value)} className={`w-full border-none rounded-xl p-3 text-base font-medium outline-none transition-all ${darkMode ? 'bg-zinc-900 text-white' : 'bg-white shadow-sm'}`} />
              </div>
            </div>

            <div className="pt-2">
              <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1 ml-1 tracking-widest">Final Sınavı (%40)</label>
              <input type="number" placeholder="0" value={scores.final} onChange={(e) => handleInputChange('final', e.target.value)} className={`w-full border-none rounded-3xl p-6 text-2xl font-bold outline-none transition-all ${darkMode ? 'bg-zinc-200 text-black focus:ring-8 focus:ring-zinc-200/20' : 'bg-zinc-700 text-white focus:ring-8 focus:ring-zinc-800/10'}`} />
            </div>
          </div>
        )}
            ...
            <>
              <div className="flex justify-between items-end w-full">
                <div className="text-left">
                  ...
                  <p className={`text-5xl font-black tracking-tighter transition-all duration-300 scale-100 ${Number(results.genelNot) >= 59.5 ? 'text-emerald-500' : ''}`}>{results.genelNot}</p>
                </div>
                <div className="text-right flex flex-col items-end">
                  <p className="text-[10px] font-bold uppercase tracking-widest opacity-60 mb-1">Gereken Final</p>
                  <p className={`text-2xl font-bold tracking-tight transition-colors duration-300 ${results.isImpossible ? 'text-red-500' : 'text-zinc-400'}`}>
                    {results.gerekenFinal}
                  </p>
                </div>
              </div>
              
              {!shareMode && (
                <button onClick={() => setShareMode(true)} className="mt-6 text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-400 hover:text-zinc-600 transition-colors flex items-center gap-2">
        ...
        {shareMode && (
          <div className="mt-8 text-center">
            ...
            <button onClick={() => setShareMode(false)} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest ${darkMode ? 'bg-white text-black' : 'bg-zinc-900 text-white'}`}>
              Geri Dön
            </button>
          </div>
        )}
